...
plot2 <- astrology_analysis %>%
  ...
  ggplot(aes(x = year, y = proportion, 
    ...
    scale_fill_manual(values = c("hotpink", "springgreen2", "steelblue3"), 
                      labels = c("18-35", "36-49", "50+")) +
    labs(
      title = "Astrology Belief Trends by Age Groups (2006-2018)",
      subtitle = "With 95% confidence intervals",
      y = "Percentage Believing",
      x = "Year",
      color = "Age Group",
...
kable(results_df, format = "html",
      col.names = c("Variable", "Coefficient", "Odd Ratio",
                    "Standard Error", "t-value", "p-value", "Significant")) %>%
...
if(nrow(year_row) > 0) {
  ...
  cat(sprintf("   - For each passing year, odds of stronger belief %s by %.1f%%\n",
...
# 4. EDUCATION EFFECT
educ_row <- results_df[results_df$variable_clean == "Education (scaled)", ]
if(nrow(educ_row) > 0) {
...
if(nrow(pol_row) > 0) {
  ...
  cat("\n6. POLITICAL VIEWS:\n")
  cat(sprintf("   • Each standard deviation toward more conservative views is associated with %.1f%% %s odds of stronger belief\n",
...
deepseek: code on ggplot2 facet grid parameters for error bars, text, color palettes, way to print the decimals of the statistics, and way to calculate p-values from scratch
